...
function renderCheckboxParts(containerId, parts, selected){
  ...
  wrap.innerHTML=Object.entries(groups).map(([group,items])=>`<div class="part-group"><h3>${group}</h3><div class="part-grid">${items.map(p=>{ const checked=selected[p.code]?'checked':''; const qty=selected[p.code]||0; return `<label class="part-chip"><span><input type="checkbox" data-part-code="${p.code}" ${checked}> <strong>${p.code}</strong> ${p.name}</span><input type="number" min="0" value="${qty}" data-part-qty="${p.code}"></label>`; }).join('')}</div></div>`).join('') || '<p class="muted">No parts loaded.</p>';
}
...
function collectCheckedParts(containerId){
  const wrap=document.getElementById(containerId); const selected={}; if(!wrap) return selected;
  wrap.querySelectorAll('[data-part-code]:checked').forEach(chk=>{ const code=chk.dataset.partCode; const qtyInput=wrap.querySelector(`[data-part-qty="${code}"]`); selected[code]=Math.max(0,parseInt(qtyInput?.value,10)||0); });
  return selected;
}
...
function saveDoorAndNext(){ const run=getRun(); saveCurrentDoor(); if(state.currentDoorIndex<run.doors.length-1){ state.currentDoorIndex++; const current=run.doors[state.currentDoorIndex]; const prev=run.doors[state.currentDoorIndex-1]; if(current.status==='Draft') Object.assign(current,{...prev,doorNumber:current.doorNumber,repairId:current.repairId,status:'Draft',notes:'',photoNotes:''}); saveLocal(); showDoor(); } else showSummary(); }
